...
Module:  catena4450_lidar_distsensor.ino
...
#include <VL53L1X.h>
#include <Adafruit_GFX.h>                             // libraries for OLED Screen
#include <Adafruit_SSD1306.h>                         // libraries for OLED Screen
#include <Adafruit_FeatherOLED.h>                     // libraries for OLED Screen
...
enum    {
        ...
        CATCFG_T_CYCLE_TEST = 10,       // every 10 seconds
        };
...
//Instance for the sensor and the OLED screen 
...
void setup()
        {
        Wire.begin();
        Wire.setClock(4000); // use 400 kHz I2C
        oled.init();
        Serial2.begin(9600);
     // Assign pins 10 & 11 SERCOM functionality
        ...
        if (!(gCatena.GetOperatingFlags() &
                ...
                if (fBme)
                        ...
                        //warm up sonar sensor
                  /*warm up sonar sensor
                  Serial2.flush();
                  int distsonarsensor = warmup_sonarsensor();
                  gCatena.SafePrintf("Warming up sonar: %d \n",distsonarsensor);*/
...
void setup_platform()
        ...
        gCatena.SafePrintf("\n");
        gCatena.SafePrintf("-------------------------------------------------------------------------------\n");
        gCatena.SafePrintf("This is the catena4470m101_sensor program V%s.\n", sVersion);
                {
                char sRegion[16];
...
void setup_sensors(void)
        ...
        if (flags & Catena::fHasLuxRohm)
                ...
                /*intialize LIDAR SENSOR*/
        if(!distanceSensor.init())
        {
          gCatena.SafePrintf("No LIDAR SENSOR found: defective board or incorrect platform\n");
          fBme = false;
          
          
        }

  // Use long distance mode and allow up to 50000 us (200 ms) for a measurement.
  ...
  // Start continuous readings at a rate of one measurement every 200 ms (the
  // inter-measurement period). This period should be at least as long as the
  // timing budget.
  distanceSensor.startContinuous(200);
        
        /*initialize Pressure Sensor BAR02  - MS5837
         *THIS IS TH PRESSURE TRANSDUCER FROM BLUE ROBOTICS THAT IS INTERFACED VIA I2C
         */
...
void loop()
        {
        gCatena.poll();
       // int sonardist = warmup_sonarsensor();
        //gCatena.SafePrintf("DiscardingDist: %d \n",sonardist);
...
void fillBuffer(TxBuffer_t &b)
        ...
        if (fLux)
                ...
                distanceSensor.read(); //Write configuration bytes to initiate measurement

                //Poll for completion of measurement. Takes 40-50ms.
/*                while (distanceSensor.newDataReady() == false)
                delay(50);*/
                uint16_t distance = distanceSensor.ranging_data.range_mm;
             ...
             /*   oled.print("LIDAR Distance : ");
                ...
                if (fdistance)
                {
                  int index, count=1,readings=0;
                  char sonardistance[6];
                  char dist[6];
                  int avg[10]; int current_dist;
                   Serial2.flush();
                 while(readings<6){//  int warmup_sonarsensor();
          //label: 
                  if(Serial2.available())
                    ...
                    {
                      dist[index]= Serial2.read();
                      //debug line
                      //Serial.print(dist[index]);
                      }
                    }
                    //else
                    //goto label;
                    ...
                    unsigned distlength = strlen(dist);
                    for(count=1;count < distlength;count ++)
                    {
                    sonardistance[count] = dist[count];
                    //Serial.println(distance[count]);    
                    }
                    current_dist = atoi(&sonardistance[1]);
                    avg[readings] = current_dist;
                    readings++;
                  ...
                  gCatena.SafePrintf("SDist: %d \n",current_dist);
                  b.putWH(current_dist);
                  b.putWH(current_dist);
                Serial2.flush();
                readings=0;
               ...
               // b.putPulseFraction(fracPower1In);
                //b.putPulseFraction(fracPower1Out);
...
/*
int warmup_sonarsensor()
        {
           int countersonar =0;
           char dummydist[6];
           int dummycurrent_dist;
           char dummysonardistance[6];
           while(countersonar < 5){
           if(Serial2.available()>0)
                    ...
                    for(int index1=0;index1<6;index1++)
                       {
                      dummydist[index1]= Serial2.read();
                      //debug line
                      //Serial.print(dist[index]);
                        }
                      }
                    }
                    unsigned distlength1 = strlen(dummydist);
                    for(int count1=1;count1 < distlength1;count1 ++)
                    {
                    dummysonardistance[count1] = dummydist[count1];
                    //Serial.println(distance[count]);    
                    }
                    dummycurrent_dist = atoi(&dummysonardistance[1]);
                    if(dummycurrent_dist==0 || dummycurrent_dist<=300 || dummycurrent_dist>=5000) 
                    {
                      Serial2.flush();
                      continue;
                    }      
